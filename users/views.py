...
class RegisterView(View):
    ...
    def post(self, request):
        ...
        role = request.POST.get("role") or RoleChioice.USER

        trimmed = username.strip()
        if not trimmed:
            messages.error(request, "Username could not be blank")
...
class LoginView(View):
    ...
    def post(self, request):
        ...
        if check_password(raw_password, user.password):
            ...
            # login success → using custom session
...
class ResetPasswordView(View):
    ...
    def get_user_from_token(self, token):
        try:
            user_id = signer.unsign(token, max_age=600)  # 10 min validity
